...
async function discoverChromePort() {
  ...
  for (const port of commonPorts) {
    ...
    if (ok) {
      ...
      // 3. 从 /json/version 获取完整 WebSocket URL（包含 UUID）
      try {
        const httpRes = await fetch(`http://127.0.0.1:${port}/json/version`);
        if (httpRes.ok) {
          const data = await httpRes.json();
          const wsUrl = data.webSocketDebuggerUrl || '';
          const match = wsUrl.match(/\/devtools\/browser\/(.+)$/);
          if (match) {
            console.log(`[CDP Proxy] 从 /json/version 获取到 wsPath: /devtools/browser/${match[1]}`);
            return { port, wsPath: `/devtools/browser/${match[1]}` };
          }
        }
      } catch { /* fetch 失败，继续 */ }
